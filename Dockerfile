...
FROM node:26.8.1-alpine AS web-builder
WORKDIR /build/web
...
ENV PYTHONDONTWRITEBYTECODE=1 \
    ...
    CAPTION_TRANSLATION_PROVIDER=none \
    CAPTION_TRANSLATION_AUTH_ROOT=/data/auth

WORKDIR /app
RUN apt-get update \
    && apt-get install --yes --no-install-recommends ffmpeg fonts-noto-cjk ca-certificates python3 python3-pip \
    && rm -rf /var/lib/apt/lists/* \
    && groupadd --system --gid 10001 studio \
    && useradd --system --uid 10001 --gid studio --home-dir /app studio \
    && mkdir -p /data/workspace /data/models /data/auth /app/web-dist \
    && chown -R studio:studio /data /app

COPY pyproject.toml README.md ./
COPY src/ ./src/
RUN npm install --global @openai/codex@0.151.0 @xai-official/grok@1.0.13 \
    && pip3 install --break-system-packages --no-cache-dir ".[asr-docker]"
...
HEALTHCHECK --interval=30s --timeout=8s --start-period=20s --retries=4 \
  CMD python3 -c "import json, urllib.request; p=json.load(urllib.request.urlopen('http://127.0.0.1:8788/api/health', timeout=5)); raise SystemExit(0 if p.get('ok') else 1)"

CMD ["python3", "-m", "subtitle_automation.web_server"]
